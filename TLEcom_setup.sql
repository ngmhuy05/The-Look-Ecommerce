-- 1. Traffic overview
CREATE OR ALTER VIEW vw_traffic_overview AS
WITH session_summary AS (
    SELECT session_id, user_id_fix AS user_id, traffic_source,
...
-- 2. Revenue & AOV theo traffic_source
CREATE OR ALTER VIEW vw_revenue_by_traffic AS
SELECT
    t3.traffic_source,
...
WHERE t1.status NOT IN ('Cancelled','Returned')
  AND t3.traffic_source IS NOT NULL
GROUP BY t3.traffic_source;
GO

-- 3. Revenue & AOV theo tháng x traffic_source
CREATE OR ALTER VIEW vw_revenue_trend_by_traffic AS
...
-- 4. Repeat rate theo traffic_source
CREATE OR ALTER VIEW vw_repeat_rate AS
WITH order_ranked AS (
    SELECT o.user_id, o.order_id, o.created_date_clean,
...
valid_orders AS (
    ...
    FROM order_ranked
    WHERE prev_order_date IS NULL OR DATEDIFF(SECOND, prev_order_date, created_date_clean) >= 60
),
user_orders AS (
    SELECT user_id, COUNT(*) AS num_orders FROM valid_orders GROUP BY user_id
)
SELECT
    u.traffic_source,
    COUNT(DISTINCT uo.user_id) AS total_buyers,
    SUM(CASE WHEN uo.num_orders >= 2 THEN 1 ELSE 0 END) AS repeat_buyers,
    CAST(SUM(CASE WHEN uo.num_orders >= 2 THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(COUNT(DISTINCT uo.user_id),0) AS repeat_rate
FROM user_orders uo
...
-- 5. Funnel tổng
CREATE OR ALTER VIEW vw_funnel_overview AS
...
-- 6. Funnel theo traffic_source
CREATE OR ALTER VIEW vw_funnel_by_traffic AS
WITH session_funnel AS (
    SELECT
        session_id,
        traffic_source,
...
-- 7. Funnel theo browser
CREATE OR ALTER VIEW vw_funnel_by_browser AS
WITH session_funnel AS (
    SELECT
        session_id,
        browser,
...
-- 8. Product-to-cart rate theo category
CREATE OR ALTER VIEW vw_product_to_cart_by_category AS
...
-- 9. Abandonment theo stock_status x category
CREATE OR ALTER VIEW vw_abandonment_by_category AS
SELECT
    sc.stock_status, pr.category,
...
JOIN (SELECT session_id, MAX(CASE WHEN event_type='purchase' THEN 1 ELSE 0 END) AS has_purchase
      FROM TLEcom_events_temp GROUP BY session_id) sp ON sp.session_id = sc.session_id
GROUP BY sc.stock_status, pr.category;
GO

-- 10. Abandonment theo traffic_source
CREATE OR ALTER VIEW vw_abandonment_by_traffic AS
...
-- 11. Abandonment theo price tier
CREATE OR ALTER VIEW vw_abandonment_by_price_tier AS
SELECT
    CASE
        WHEN pr.retail_price < 25 THEN '< $25'
        WHEN pr.retail_price < 50 THEN '$25 - $50'
        WHEN pr.retail_price < 100 THEN '$50 - $100'
        ELSE '>= $100'
    END AS price_tier,
    CASE
        WHEN pr.retail_price < 25 THEN 1
        WHEN pr.retail_price < 50 THEN 2
        WHEN pr.retail_price < 100 THEN 3
        ELSE 4
...
-- 12. Revenue at risk theo category
CREATE OR ALTER VIEW vw_revenue_at_risk AS
SELECT
    pr.category,
